...
サイト数がわずか4であるため、ヒルベルト空間は $2^4=16$ 次元であり、どのような対角化手法でも瞬時に計算できます。ここで `sparsediag` の Lanczos アルゴリズムを用いるのは、他の厳密対角化チュートリアルとの一貫性を保つため、そしてより大きな系にも拡張できるカスタム格子・カスタムモデルのワークフローを示すためです。
