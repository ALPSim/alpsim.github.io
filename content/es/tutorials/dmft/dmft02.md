...
### Entendiendo los parámetros de entrada
...
Observe que no hay ningún parámetro que especifique la estructura de bandas o el tipo de red. Por defecto se asume una red de Bethe, pero esto puede cambiarse (ver [DMFT-08 Redes](../dmft08)).

También falta una especificación del campo de Weiss inicial (fijado por las variables G0OMEGA_INPUT o G0TAU_INPUT); el programa, por tanto, calculará al inicializar la función de Green no interactuante. Utilizará el campo magnético inicial H_INIT, que produce en este caso una pequeña diferencia entre los "flavors" (0 y 1, que representan $\uparrow$ y $\downarrow$) para partir de un punto alejado de la solución paramagnética. La razón es que en simulaciones muy cortas (como este tutorial), partir de un campo de Weiss paramagnético podría significar que el ruido aleatorio no produjera suficiente diferencia en las primeras iteraciones para alejar al sistema del régimen paramagnético; un paramagneto mal convergido aparecería entonces como una solución. La dependencia de H_INIT con BETA sirve para optimizar la ejecución, reduciendo el número de iteraciones necesarias.

### Iterando hacia la autoconsistencia

El código ejecutará hasta 6 iteraciones de autoconsistencia. Para una simulación más precisa se puede aumentar este número; la simulación se detendrá antes en cuanto se alcance el criterio de convergencia especificado por el parámetro CONVERGED. En el directorio en el que se ejecuta el programa encontrará archivos de funciones de Green `G_tau_i`, las autoenergías (`selfenergy_i`), y las funciones de Green en representación de Matsubara (espacio de frecuencias) `G_omega_i`. `G_tau` en estos ejemplos tiene dos columnas: una de espín arriba y otra de espín abajo. La entrada en $\tau=\beta^-$ es la ocupación negativa (densidad); a partir de ella podemos obtener la magnetización del sistema.

Las barras de error pueden estimarse mediante iteraciones sucesivas sobre un sistema convergido.

Para volver a ejecutar una simulación, puede especificar una solución de partida definiendo el parámetro de entrada G0OMEGA_INPUT: copie el `G0omega_output` deseado a `filename_X` y especifique `'G0OMEGA_INPUT':'filename_X'` en el script de python (o `G0OMEGA_INPUT=filename_X` directamente en el archivo de entrada), y vuelva a ejecutar el código.

### Graficando la transición antiferromagnética

Como en la Fig. 11 de la [revisión de DMFT de Georges et al.](https://journals.aps.org/rmp/abstract/10.1103/RevModPhys.68.13), puede observar la transición a la fase antiferromagnética graficando las funciones de Green en la representación de tiempo imaginario (parte de `tutorial2.py` y `tutorial2_long.py`):
...
Notará que los resultados son relativamente ruidosos. Esto se debe a que el orden de expansión a temperaturas tan altas es muy pequeño, lo que hace que el procedimiento de medición sea ineficiente. Puede mejorar la estadística aumentando el tiempo total de ejecución (MAX_TIME) y/o el número de SWEEPS. Tenga en cuenta que ejecutar el solucionador directamente bajo MPI mediante el parámetro SOLVER (por ejemplo `SOLVER = "mpirun -np procs /path-to-ALPS-installation/bin/hybridization"`) actualmente no funciona, debido a un problema con el prefijo de la ruta; consulte la documentación de su instalación de MPI (por ejemplo, la página de manual de `mpirun`) para la forma correcta de lanzar trabajos paralelos en su sistema.
...
Cabe destacar que los resultados resueltos por iteración se cargan con una función distinta (`pyalps.loadDMFTIterations`) a la de los resultados finales (`pyalps.loadMeasurements`), porque los datos resueltos por iteración se almacenan usando una estructura de carpetas (`/simulation/iteration/number/results/`) distinta de la usada por defecto en ALPS (`/simulation/results/`) para el almacenamiento de los resultados finales.

Como ya se mencionó, la ocupación $n_f$ es igual a $G_f(\tau=\beta^-)$, la última entrada de la función de Green en tiempo imaginario para el flavor $f$. El código para imprimir las ocupaciones finales y graficarlas frente a $\beta$ forma parte de `tutorial2eval.py`:
...
### Función de Green en frecuencias de Matsubara y autoenergía
