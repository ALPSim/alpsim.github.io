---
title: Biblioteca ALPS/Parser
description: "Biblioteca ALPS Parser"
weight: 5
---
...
lee una etiqueta XML o un nombre de atributo desde un `std::istream`

#### `public std::string `[`parse_parameter_name`](#d3/df5/parser_8_c_1a7c2085959fd8e42215655c4525ed3d45)`(std::istream & in)` 

lee un nombre de parámetro ALPS desde un `std::istream`

los caracteres válidos, además de los de un identificador XML, son `'`, y adicionalmente cualquier secuencia arbitraria de caracteres (incluyendo espacios en blanco) rodeada por los caracteres ``[ ... \ ]`, como en `MEASURE`[Staggered `Magnetization^2`] .

#### `public std::string `[`read_until`](#d3/df5/parser_8_c_1a9c6396ca9b36dae30b589d285b4da6d9)`(std::istream & in,char end)` 

lee hasta la siguiente aparición del carácter *end* o hasta que se alcanza el final del flujo.

#### Parámetros
* `in` el flujo que se debe leer 

* `end` el carácter hasta el cual se debe leer 

#### Devuelve
cadena que contiene los caracteres leídos, excluyendo los espacios en blanco iniciales y finales y excluyendo el carácter terminador *end*.
...
comprueba el siguiente carácter leído del flujo.

#### Parámetros
* `in` el flujo que se debe leer 

* `c` el carácter que se debe leer 

* `err` el mensaje de error que se usará si el siguiente carácter no es *c*. 

#### Excepciones
* 

c std::runtime_error( *err*``) si el siguiente carácter no es *c*, lee el siguiente carácter (saltando espacios en blanco) y comprueba si es igual al carácter pasado como argumento *c*, y lanza un `std::runtime_error` en caso contrario.
...
analiza una etiqueta XML

#### Parámetros
* `in` el flujo que se debe leer 

* `skip_comments` si es true, la función omite cualquier comentario o instrucción de procesamiento durante el análisis 

#### Devuelve
una estructura `[XMLTag](#df/df1/structalps_1_1_x_m_l_tag)` que contiene información sobre la etiqueta

#### `public std::string `[`parse_content`](#d3/df5/parser_8_c_1a3704a802259862e15a2d2cffc3ae5a66)`(std::istream & in)` 

lee el contenido de un elemento, hasta encontrar el primer carácter <
...
omite un elemento XML

#### Parámetros
* `in` el flujo que se debe leer 

* `tag` la etiqueta de apertura del elemento a omitir; la función lee hasta encontrar la etiqueta de cierre correspondiente a la *tag* pasada como argumento.

#### `public void `[`check_tag`](#d3/df5/parser_8_c_1acbaa7b898feeade68469f8f461e0bcb9)`(std::istream & in,const std::string & name)` 

comprueba si la siguiente etiqueta en el archivo XML tiene el nombre dado

#### Parámetros
* `in` el flujo que se debe leer 

* `name` el nombre de la etiqueta esperada 

#### Excepciones
* 

c std::runtime_error si la siguiente etiqueta leída del flujo *in* no tiene el nombre dado por el argumento *name*.
...
dado el nombre de un archivo XSLT, devuelve la ruta completa

el comportamiento por defecto es simplemente devolver el nombre del archivo

si la variable de entorno ALPS_XSLT_PATH está definida, se usa el contenido de ALPS_XSLT_PATH en lugar de la ruta por defecto.
...
devuelve la ruta completa al archivo XML especificado y comprueba si el archivo existe.

La función antepone la ruta al directorio de la biblioteca XML/XSLT de ALPS al nombre de archivo especificado. 
#### Excepciones
* 

c std::runtime_error si el archivo no existe en el directorio de la biblioteca XML/XSLT de ALPS.
...
copia la hoja de estilo ALPS.xsl al directorio especificado

Esta función copia la hoja de estilo ALPS.xsl al directorio especificado. La función no sobrescribe un archivo ya existente con el nombre ALPS.xsl
...
una estructura para almacenar el contenido de una etiqueta XML

## Resumen

 Miembros                        | Descripciones                                
--------------------------------|---------------------------------------------
`public std::string `[`name`](#df/df1/structalps_1_1_x_m_l_tag_1a169079bc35d40c7bdb3cf8c53dda3ad3) | el nombre de la etiqueta
`public `[`XMLAttributes`](#db/d77/classalps_1_1_x_m_l_attributes)` `[`attributes`](#df/df1/structalps_1_1_x_m_l_tag_1aff8ed559f5246cbed1dd5394e2a8894d) | los atributos
...
`public inline bool `[`is_comment`](#df/df1/structalps_1_1_x_m_l_tag_1a7643c7cb52671972ad3030d3dd1131bc)`()` | devuelve true si la etiqueta es un comentario
`public inline bool `[`is_processing`](#df/df1/structalps_1_1_x_m_l_tag_1a06b3d21bb5fd20916aaff872f6b9720b)`()` | devuelve true si la etiqueta es una instrucción de procesamiento
`public inline bool `[`is_element`](#df/df1/structalps_1_1_x_m_l_tag_1a89b77b175de112861bc5becc7a62ae11)`()` | devuelve true si la etiqueta es una etiqueta de apertura o cierre de un elemento XML
...
## Resumen
 ...
 Miembros                        | Descripciones                                
--------------------------------|---------------------------------------------
`public std::string `[`parse_string`](#d3/df5/parser_8_c_1a105e995b91c122797b5c89a9b79fe783)`(std::istream & in)`            | lee una cadena, delimitada por comillas dobles "
`public std::string `[`xml_parse_name`](#d3/df5/parser_8_c_1a0f5b4b2996ae026cfb1b88c1677b1765)`(std::istream & in)`            | lee cualquier nombre de etiqueta o atributo XML válido
`public void `[`xml_read_attribute`](#d3/df5/parser_8_c_1ae6bf0cb8e50637f5cd70c9b7fd670d56)`(std::istream & in,std::string & name,std::string & value)`            | analiza un atributo XML
`public std::string `[`xml_read_tag`](#d3/df5/parser_8_c_1a98d374051a313ce1cc88bb872199f2c1)`(std::istream & in)`            | analiza el < de apertura y el nombre de una etiqueta
`public void `[`xml_close_tag`](#d3/df5/parser_8_c_1a010b8e93c5629a33ecf6a507abd79333)`(std::istream & in)`            | comprueba el cierre > de una etiqueta
`public void `[`xml_close_single_tag`](#d3/df5/parser_8_c_1a310d3c43e0d76e6e67e9e45b926fddb2)`(std::istream & in)`            | comprueba el cierre /> de una etiqueta
`public void `[`skip_comment`](#d3/df5/parser_8_c_1add18575e1a12aa44db7d05079474ac70)`(std::istream & in,bool processing)`            | omite un comentario XML o una instrucción de procesamiento
...
omite un comentario XML o una instrucción de procesamiento
