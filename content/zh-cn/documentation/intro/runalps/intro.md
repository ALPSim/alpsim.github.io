...
每当运行蒙特卡洛模拟时，都要记住你所使用的是伪随机数。总有极小的概率，你的特定应用恰好是暴露出某个此前一直表现良好的伪随机数生成器缺陷的那一个。因此，作为高精度蒙特卡洛计算的标准做法，你应当用另一个生成器——通过 [`RNG` 参数](../../parameters#additional-parameters-for-monte-carlo-simulations) 设置——重新运行模拟，并检查结果是否保持不变。
