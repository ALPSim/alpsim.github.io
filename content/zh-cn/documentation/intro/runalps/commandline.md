...
| **参数** | **默认值** | **含义** |
| :------------ | :---------- | :---------- |
| SEED | 0 | 下一次创建的蒙特卡洛运行所使用的伪随机数生成器种子。每当一个种子被用于创建一次运行后，该值会自动加一。 |
...
该程序接受若干命令行选项，用于控制调度器的行为。这些选项在蒙特卡洛模拟中尤其有用。
...
如果可用的处理器数目多于模拟的数目，每次模拟会启动不止一个蒙特卡洛运行。
...
模拟的输出文件中只包含来自所有运行的汇总测量结果。若要获得每次模拟中各个蒙特卡洛运行的详细信息，可以使用 `convert2xml` 工具将检查点文件转换为 XML，例如：

    convert2xml run-file

这会生成该任务的一个 XML 文件，其中包含从这次蒙特卡洛运行中提取出的信息。
...
这将利用保存下来的蒙特卡洛数据，计算模拟过程本身未曾计算的其他物理量（例如比热、压缩率等）。使用 `--write-xml` 会将所有结果写回 `.out.xml` 文件；不使用该选项时，结果只会写入 HDF5 文件。
