...
两个位于相邻原子上的电子，通过普通的库仑相互作用彼此排斥，这种相互作用本身完全不关心自旋。但由于电子是费米子，泡利不相容原理要求：如果两个电子的自旋组合成反对称的单态，它们的空间波函数就必须是对称的；如果自旋组合成对称的三重态，空间波函数就必须是反对称的——而这两种空间波函数具有不同的库仑能量。把问题中的空间部分消去之后，剩下的就是两个电子之间纯粹依赖自旋的有效相互作用：**交换相互作用**，由 Werner Heisenberg 于 1928 年提出（[Heisenberg (1928)](https://doi.org/10.1007/BF01328601)），作为固体中磁性的微观起源。这种相互作用纯粹是库仑相互作用，加上电子作为不可区分费米子这一量子力学要求所带来的副产物。
...
- 在**经典海森堡模型**中，$\mathbf{S}_i$ 是一个普通的三分量单位矢量，可以指向空间中任意方向，$\mathbf{S}_i \cdot \mathbf{S}_j$ 就是日常意义下的点积。这是一个统计力学模型，用经典蒙特卡洛方法模拟。
...
**本网站主要关注量子版本。** ALPS 的精确对角化、DMRG 以及量子蒙特卡洛程序都以量子海森堡模型为目标，下文链接的大多数教程也都是围绕它展开的；经典模型主要作为对比出现（参见下文的 [MC-02 教程](../../../tutorials/mcs/mc02)，它将两者并列模拟），由经典蒙特卡洛程序 `spinmc` 处理，这也正是[伊辛模型](../ising)所使用的同一个程序。
...
**量子蒙特卡洛（`looper`、`dirloop_sse`、量子王-朗道）：**
- [MC-02：用经典 MC 和 looper QMC 程序计算磁化率](../../../tutorials/mcs/mc02) —— 直接并列比较经典与量子海森堡模型
- [MC-03：用有向圈量子蒙特卡洛程序计算磁化曲线](../../../tutorials/mcs/mc03) —— 链和有能隙梯子在外场驱动下的磁化行为
- [MC-04：QMC 程序中的自定义测量](../../../tutorials/mcs/mc04) —— 自旋-自旋关联函数与反铁磁结构因子
- [MC-06：扩展系综模拟（量子王-朗道）](../../../tutorials/mcs/mc06) —— 单次模拟即可得到海森堡链和梯子完整的有限温度热力学
