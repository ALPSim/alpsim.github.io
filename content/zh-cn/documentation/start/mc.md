...
二维伊辛模型是统计力学中最重要的模型之一。它描述了正方格子上的自旋，每个自旋只能向上或向下，相邻自旋之间存在铁磁耦合 $J > 0$。Onsager 于 1944 年证明该模型存在精确解，并在 $T_c = 2J / \ln(1 + \sqrt{2}) \approx 2.269\, J/k_B$ 处发生相变：低于 $T_c$ 时自旋自发有序，高于 $T_c$ 时热涨落破坏有序态。
