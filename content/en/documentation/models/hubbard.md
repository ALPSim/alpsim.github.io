...
Electrons hopping on a lattice would, according to ordinary band theory, simply spread out to minimize their kinetic energy. But when two electrons try to occupy the same atomic orbital, they also feel the full strength of the Coulomb repulsion between them. The **Hubbard model** keeps only this single, essential piece of the electron-electron interaction — an energy cost $U$ for any doubly-occupied site — on top of ordinary single-particle hopping, and in doing so becomes the simplest possible model of interacting electrons on a lattice. In the cold-atom and optical-lattice communities it is often called the **Fermi-Hubbard model**, to distinguish it explicitly from its bosonic counterpart, the [Bose-Hubbard Model](../bhm), since both are realized experimentally with ultracold atoms trapped in optical lattices, differing only in the quantum statistics of the atoms used. Despite (or perhaps because of) this simplicity, it captures some of the most important phenomena in condensed matter physics: the Mott metal-insulator transition, itinerant magnetism, and, in its doped low-energy limit, some of the physics believed to underlie high-temperature superconductivity in the cuprates.
