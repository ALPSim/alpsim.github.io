...
We start by running a continuous-time quantum Monte Carlo code: the hybridization-expansion algorithm CT-HYB. As an example we reproduce Fig. 11 in the DMFT review by [Georges et al., Rev. Mod. Phys. 68, 13 (1996)](https://journals.aps.org/rmp/abstract/10.1103/RevModPhys.68.13). The series of six curves shows how the system, a Hubbard model on the Bethe lattice with interaction $U=3D/\sqrt{2}$ at half filling, enters an antiferromagnetic phase upon cooling. In Tutorials 03 and 07 we will reproduce the same results with the interaction-expansion continuous-time solver and with the discrete-time Hirsch-Fye Quantum Monte Carlo code, respectively. The input parameters are the same, apart from a few solver-related parameters.
...
All DMFT tutorials can be started using a python script. The python script generates parameter files, runs them, and plots the results. You can run the short script <a href="../codes/dmft-02-hybridization/tutorial2.py" download>`tutorial2.py`</a>, reproducing only 2 out of the 6 curves (runtime: roughly 20 minutes), or the long version <a href="../codes/dmft-02-hybridization/tutorial2_long.py" download>`tutorial2_long.py`</a>, reproducing all 6 curves in the figure (runtime: roughly 1 hour).
...
Note that there is no parameter specifying the band structure or lattice type. By default a Bethe lattice is assumed, but this can be changed (see [DMFT-08 Lattices](../dmft08)).

A specification of the initial Weiss field (set by the variables G0OMEGA_INPUT or G0TAU_INPUT) is missing as well - the program will thus at initialization compute the non-interacting Green's function. It will use the initial magnetic field H_INIT, which produces in this case a small difference between flavors (0 and 1, representing $\uparrow$ and $\downarrow$) to start away from the paramagnetic solution. The reason for this is that in very short simulations (like this tutorial), starting from a paramagnetic Weiss field could mean that the random noise would not produce enough difference in the first few iterations to move the system away from the paramagnetic regime; a badly converged paramagnet would then appear as a solution. The dependence of H_INIT on BETA serves to optimize the run, lowering the number of iterations needed.

### Iterating to self-consistency

The code will run for up to 6 self-consistency iterations. For a more precise simulation you can raise this number; the simulation will stop early once the convergence criterion specified by the parameter CONVERGED is reached. In the directory in which you run the program you will find Green's function files `G_tau_i`, the self-energies (`selfenergy_i`), and Green's functions in Matsubara representation (frequency space) `G_omega_i`. `G_tau` in these examples has two entries: a spin-up and a spin-down column. The entry at $\tau=\beta^-$ is the negative occupation (density); from this we can obtain the magnetization of the system.

Error bars may be estimated from successive iterations on a converged system.

To rerun a simulation, you can specify a starting solution by defining the input parameter G0OMEGA_INPUT: copy the desired `G0omega_output` to `filename_X` and specify `'G0OMEGA_INPUT':'filename_X'` in the python script (or `G0OMEGA_INPUT=filename_X` in the input file directly), then rerun the code.

### Plotting the antiferromagnetic transition

As in Fig. 11 of the DMFT review by [Georges et al.](https://journals.aps.org/rmp/abstract/10.1103/RevModPhys.68.13), you can observe the transition to the antiferromagnetic phase by plotting the Green's functions in the imaginary-time representation (part of `tutorial2.py` and `tutorial2_long.py`):
...
You will notice that the results are relatively noisy. This is because the expansion order at such high temperatures is very small, which makes the measurement procedure inefficient. You can improve statistics by increasing the total runtime (MAX_TIME) and/or the number of SWEEPS. Note that running the solver directly under MPI via the SOLVER parameter (e.g. `SOLVER = "mpirun -np procs /path-to-ALPS-installation/bin/hybridization"`) currently does not work, due to an issue with the path prefix; consult your MPI installation's documentation (e.g. the `mpirun` man page) for the correct way to launch parallel jobs on your system.

### Checking convergence

If you want to check the convergence of your DMFT self-consistency, you can plot the Green's functions of different iterations using <a href="../codes/dmft-02-hybridization/tutorial2eval.py" download>`tutorial2eval.py`</a>, whose code is shown here:
...
Note that the iteration-resolved results are loaded by a different function (`pyalps.loadDMFTIterations`) than the final results (`pyalps.loadMeasurements`), because the iteration-resolved data is stored using a different folder structure (`/simulation/iteration/number/results/`) than the ALPS default (`/simulation/results/`) used for storing the final results.

As mentioned above, the occupation $n_f$ is equal to $G_f(\tau=\beta^-)$, the last entry of the imaginary-time Green's function for flavor $f$. Code for printing the final occupancies and plotting them versus $\beta$ is part of `tutorial2eval.py`:
...
### Matsubara-frequency Green's function and self-energy

Since our self-consistency runs in Matsubara frequencies (recall parameter OMEGA_LOOP=1), the convergence criterion is $\mathrm{max}|G_{f}^{it}(i\omega_n)-G_{f}^{it+1}(i\omega_n)|\lt$ CONVERGED. The imaginary part (the real part analogously) of the Matsubara-frequency Green's function is plotted by
...
It is usually best to observe convergence in the self-energy, which is much more sensitive. Note that longer simulations are required to obtain smoother Green's functions and self-energies; in this simulation the noise in the intermediate range of Matsubara frequencies is very strong. The self-energy is obtained via the Dyson equation as $\Sigma_f^{it}(i\omega_n)=G0_f^{it}(i\omega_n)^{-1}-G_f^{it}(i\omega_n)^{-1}$, and its imaginary part is plotted by this fragment of `tutorial2eval.py` (real part analogously):
