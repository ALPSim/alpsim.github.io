...
The Hirsch-Fye simulation will run for about 20 seconds per iteration. The files for this tutorial can be found in the directory `tutorials/dmft-07-hirschfye`. As in Tutorials 02 and 03, you can run the short script <a href="../codes/dmft-07-hirschfye/tutorial7.py" download>`tutorial7.py`</a>, reproducing 2 of the 6 curves (runtime: roughly 5 minutes), or the long version <a href="../codes/dmft-07-hirschfye/tutorial7_long.py" download>`tutorial7_long.py`</a>, reproducing all 6 curves.
...
For evaluation you may adapt `tutorial2eval.py` as described in [DMFT-02 Hybridization](../dmft02), or use <a href="../codes/dmft-07-hirschfye/tutorial7eval.py" download>`tutorial7eval.py`</a>, which is structurally identical to `tutorial2eval.py`: it plots the iteration-resolved $G(\tau)$, the occupation $n_0=-G_0(\tau=\beta^-)$ versus $\beta$, and the Matsubara-frequency Green's function and self-energy (via the Dyson equation), for both flavors.
