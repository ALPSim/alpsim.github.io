...
<SITEBASIS name="my basis">
  ...
  <OPERATOR name="Raise"                  
            matrixelement="sqrt(P*(P+1)-Q*(Q+1))">  <!-- off-diagonal creation operator     -->
...
### Step 2 — HAMILTONIAN: on-site and bond terms

```xml
<HAMILTONIAN name="my model">
  <PARAMETER name="J" default="1"/>
  <SITEBASIS ref="my basis"/>         <!-- which local space to use      -->
  <SITETERM site="i">
...
```xml
<MODELS>
  <!-- one or more SITEBASIS elements -->
  <!-- one or more HAMILTONIAN elements -->
</MODELS>
```
...
At $D = 0$ the chain is in the **Haldane phase**: a gapped, symmetry-protected topological phase with a bulk spin gap $\Delta \approx 0.41\,J$.
...
`N_total=5` fills half the chain (half-filling, one fermion per two sites), where the CDW instability is strongest.
Use `NUMBER_EIGENVALUES=4` to obtain the ground state and three excited states and inspect the charge gap.
...
Compute the single-particle gap $\Delta = E(N+1) + E(N-1) - 2E(N)$ by running additional tasks with `N_total=5` and `N_total=7`, analogously to the spin gap calculation above.
